use crate::{
    proxy::{GeneralProxy, Proxy, ProxySettings},
    target::ProxyTarget,
};
...
impl Bash {
    fn generate_bashrc_content(settings: &[&ProxySettings]) -> String {
...
impl ProxyTarget for Bash {
    fn get(&self) -> Option<Vec<ProxySettings>> {
        let bashrc_file = std::fs::read_to_string("~/.bashrc").unwrap_or_default();
        let mut proxy_settings = ProxySettings::default();

        for line in bashrc_file.lines() {
            let line = line.trim();
    ...
    fn set(&self, settings: Vec<&ProxySettings>) -> Result<(), Box<dyn Error>> {
        let content = Bash::generate_bashrc_content(&settings);
